...
[verse]
// tag::synopsis[]
aerospace fullscreen [-h|--help]     [--window-id <window-id>] [--no-outer-gaps] [--frame <x,y,w,h>]
aerospace fullscreen [-h|--help] on  [--window-id <window-id>] [--no-outer-gaps] [--frame <x,y,w,h>] [--fail-if-noop]
...
--frame <x,y,w,h>::
Hold the window at a custom rectangle instead of the whole monitor.
`x,y,w,h` are fractions (0-1) of the monitor's visible area (e.g. `0.1,0.1,0.8,0.8`).
The window stays in the tiling tree, so other windows don't move
